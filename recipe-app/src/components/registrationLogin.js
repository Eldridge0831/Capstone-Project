...
function RegistrationLogin() {
  ...
  /*----Login Validation-----*/
    ...
    function registerUser() {
      fetch("http://localhost:3001/users", // this is place holder TODO
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          role: "user", //keep if admin page is created otherwise remove
        }),
      })
        .then((res) => res.json())
        .then((res) => {
          ...
          <div className="container border border-primary border-2 mt-5 ">
            <ul className="nav nav-tabs p-3" id="myTab" role="tablist">
              <li className="nav-item" role="presentation">
                <button className="nav-link active" id="home" data-bs-toggle="tab" data-bs-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">
                  Login
                </button>
              </li>
              <li className="nav-item" role="presentation">
                <button className="nav-link" id="profile" data-bs-toggle="tab" data-bs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="false">
                  Register
                </button>
              </li>
            </ul>
            <div className="tab-content" id="myTabContent">
              ...
              <div className="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                ...
                >
                  <div className="mb-3 text-start">
                    <label for="EnterFirstName" className="form-label">
                      First Name
                    </label>
                    <input type="text" className="form-control" id="EnterFirstName" required />
                  </div>
                  <div className="mb-3 text-start">
                    <label for="EnterLastName" className="form-label">
                      Last Name
                    </label>
                    <input type="text" className="form-control" id="EnterLastName" required />
                  </div>
                  <div className="mb-3 text-start">
                    <label for="EnterUserName" className="form-label">
                      Username 
                    </label>
                    <input type="text" className="form-control" id="EnterUserName" required />
                  </div>{" "}
                  <div className="mb-3 text-start">
                    <label for="EnterEmail" className="form-label">
                      Email Address
                    </label>
                    <input type="email" className="form-control" id="EnterEmail" required />
                  </div>
                  
                  <div className="mb-3 text-start">
                    <label for="EnterPassword" className="form-label">
                      Password
                    </label>
                    <input type="password" className="form-control" id="EnterPassword" required />
                  </div>
                  <button type="submit" className="btn btn-primary">
                    Submit
                  </button>
